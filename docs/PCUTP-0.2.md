...
PicoCalc が起呼側、uConsole が応答側である。以下は上から下へ時間が進む ASCII シーケンス図である。最初の `HELLO` は受信能力の提示、応答の `HELLO ... HRU?` は合意案、最後の `HRU` は受諾である。
...
```text
       ...
       receiver                                       sender
          ...
          |                                              |
          |                               +--------------+--------------+
          |                               | Check version: PCUTP/2      |
          |                               | Select common capabilities  |
          |                               +--------------+--------------+
          |                                              |
          |  (2) HELLO PCUTP/2 HRU?                      |
...
現行実装は能力交渉で **`IDENTITY=1`** が双方から提示・合意されたときだけ `WHO?` と `IAM` を使う。能力未合意のピアへ送ることはプロトコルエラーを招く。
