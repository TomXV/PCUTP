...
FETCHINGと45秒の専用タイムアウトを使う。

キープアライブ音は短いモールス風パターンにする。BEACONはB (`-...`)、PINGはP
(`.--.`)、PONGはPにO (`---`)を続けた`PO`として鳴らす。uConsoleはパターン全体を
事前生成したPCMで再生し、通信スレッドを止めない。PicoCalcはPONGをUARTへ送ってから
パターンを鳴らすため、音によって応答が遅延しない。

接続と切断も同じ規則で、HELLOはH (`....`)、再送HELLO?は疑問符 (`..--..`)、
OHRUはO (`---`)、HRUは文字列HRU (`.... .-. ..-`)、CLOSEはC (`-.-.`)、BYEは
B (`-...`) とする。同じ制御語でも、受信側では低音、送信側では1オクターブ高い音で
鳴らす。応答を伴うHRU、PONG、BYEは必ずUARTへ送信してから音を再生する。
