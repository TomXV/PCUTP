...
OSI参照モデルに当てはめると、PicoCalc側にはネットワーク層もトランスポート層もありません。その範囲をまるごとuConsoleへ肩代わりさせるのがPCUTPです。
...
PicoCalc側に必要なのは **UART・ファイルシステム・CRC32・簡単な行解析** だけで、Wi-Fiドライバ、TCP/IPスタック、DNSリゾルバ、HTTPクライアント、TLS実装、証明書処理はいずれも不要です。
...
一方、ファイル本体はそのままバイナリデータとして転送します。データブロックは、制御行のすぐ後ろに生バイト列が続く形です。
...
受信側は改行を探さず、制御行が宣言した `len` バイトを数えて読みます。そのため `0x00` / `0x0A` / `0x0D` / `0xFF` のような値を含むPNG、ZIP、BINなどの任意のバイナリファイルも安全に転送できます。
...
ブロック単位のCRC32は「どこが壊れたか」を特定してそのブロックだけ送り直すため、ファイル全体のCRC32は「出来上がったファイルが正しいか」を確かめるためのものです。役割が違うので、二段階で確認します。
